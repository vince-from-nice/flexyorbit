...
import { addSlider, addCheckbox, addCustomSelect, addSubPanel, addNonEditableText, addEditableText, addColorPicker } from './widgets.js';
...
function buildStaticEntityPanel() {
    ...
    entityWidgets.type = addNonEditableText(infos, 'Type', '');
    entityWidgets.status = addNonEditableText(infos, 'Status', '', '#0f9');
    ...
    entityWidgets.posX = addNonEditableText(worldGrp, 'X', '', null, 'km');
    entityWidgets.posY = addNonEditableText(worldGrp, 'Y', '', null, 'km');
    entityWidgets.posZ = addNonEditableText(worldGrp, 'Z', '', null, 'km');
    ...
    entityWidgets.vx = addNonEditableText(vel, 'Vx', '', null, 'km/s');
    entityWidgets.vy = addNonEditableText(vel, 'Vy', '', null, 'km/s');
    entityWidgets.vz = addNonEditableText(vel, 'Vz', '', null, 'km/s');
    entityWidgets.speed = addNonEditableText(vel, 'Speed', '', null, 'km/s');
    ...
    entityWidgets.totalAcc = addNonEditableText(accContainer, 'Total', '', null, 'm/s²');
    const gRow = makeAccelerationRow(accContainer);
    entityWidgets.gravAcc = addNonEditableText(gRow, '- Gravity', '', null, 'm/s²');
    ...
    entityWidgets.dragAcc = addNonEditableText(dRow, '- Friction', '', null, 'm/s²');
    ...
    entityWidgets.engineAcc = addNonEditableText(eRow, '- Engine', '', null, 'm/s²');
...
function makeAccelerationRow(container) {
    ...
    Object.assign(row.style, { display: 'flex', justifyContent: 'space-between', alignItems: 'right', width: '100%', minWidth: '100%', boxSizing: 'border-box' });
...
export function updateEntityWidgets() {
    ...
    entityWidgets.totalAcc.textContent = fmt(entity.accelerations.total.length() * 1000);
    entityWidgets.gravAcc.textContent = fmt(entity.accelerations.gravity.length() * 1000);
    entityWidgets.showGrav.checked = entity.vectors.showAccelerationForGravity;
    entityWidgets.dragAcc.textContent = fmt(entity.accelerations.friction.length() * 1000);
    entityWidgets.showDrag.checked = entity.vectors.showAccelerationForDrag;
    entityWidgets.engineAcc.textContent = fmt(entity.accelerations.engine.length() * 1000);
